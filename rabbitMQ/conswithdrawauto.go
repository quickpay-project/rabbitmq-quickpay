...
func getRandomWithdrawautoURLByGroup(headers map[string]interface{}) (string, error) {
	...
	envKey := "WITHDRAW_AUTO_URL_GROUP" + group
...
func sendToExternalWithdrawautoAPI(data []byte, headers map[string]interface{}) (int, string, string, []string, error) {
	apiURL, err := getRandomWithdrawautoURLByGroup(headers)
	if err != nil {
		return 0, "", "", nil, err
	}

	log.Printf("🌐 Withdraw Auto API URL (Group): %s", apiURL)
