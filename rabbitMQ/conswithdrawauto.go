...
func getRandomWithdrawautoURL(_ map[string]interface{}) (string, error) {
	envValue := os.Getenv("WITHDRAW_AUTO_URL_GROUP")
	if envValue == "" {
		return "", errors.New("WITHDRAW_AUTO_URL_GROUP not set")
	}

	urls := strings.Split(envValue, ",")
	if len(urls) == 0 {
		return "", errors.New("no valid URLs in WITHDRAW_AUTO_URL_GROUP")
	}
...
func sendToExternalWithdrawautoAPI(data []byte, headers map[string]interface{}) (int, string, string, []string, error) {
	apiURL, err := getRandomWithdrawautoURL(headers)
	if err != nil {
		return 0, "", "", nil, err
	}

	log.Printf("🌐 Withdraw Auto API URL: %s", apiURL)
	...
	var withdrawResp WithdrawautoResponse
...
func (r *RabbitWithdrawautoMQ) ConswithdrawautoRPC() {
	...
	for i := 0; i < workerCount; i++ {
		go func(workerID int) {
			for d := range msgs {
				...
				httpStatus, respBody, firstTxnID, txnIDs, sendErr := sendToExternalWithdrawautoAPI(d.Body, headers)
				...
				_, _ = insertWithdrawautoLog(r.QueueName, d.Body, headers, httpStatus, respBody, status, 1, errMsg, firstTxnID)
