...
type RabbitDepositautoMQ struct {
	Body      string
	QueueName string
...
func genCorrelationDepositautoID() string {
	b := make([]byte, 16)
	rand.Read(b)
...
// ========== RPC CLIENT ==========
func (r *RabbitDepositautoMQ) DepositautoRPC() ([]byte, error) {
	conn, ch := ConnectMQ()
	...
	corrID := genCorrelationDepositautoID()
